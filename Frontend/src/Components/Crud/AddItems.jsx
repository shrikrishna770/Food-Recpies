                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="flex mt-[30px] gap-[30px]">
                            <div className="flex flex-col w-[50%] gap-[5px]">
                                ...
                                <input type="text" required value={prepTime} onChange={(e) => setPrepTime(e.target.value)} placeholder="e.g. 15 min" className="border border-gray-300 outline-none rounded-lg h-[40px] pl-[10px]" />
                            </div>
                            <div className="flex flex-col w-[50%] gap-[5px]">
                                <label>Cook Time</label>
                                <input type="text" required value={cookTime} onChange={(e) => setCookTime(e.target.value)} placeholder="e.g. 30 min" className="border border-gray-300 outline-none rounded-lg h-[40px] pl-[10px]" />
                            </div>
                        </div>
                        ...
                        <div className="flex mt-[30px] gap-[30px]">
                            <div className="flex flex-col w-[50%] gap-[5px]">
                                <label>Serving</label>
                                <input type="number" required value={servings} onChange={(e) => setServings(e.target.value)} placeholder="e.g. 4" className="border border-gray-300 outline-none rounded-lg h-[40px] pl-[10px]" />
                            </div>
                            <div className="flex flex-col w-[50%] gap-[5px]">
                                <label>Calories (per serving)</label>
                                <input type="text" required value={calories} onChange={(e) => setCalories(e.target.value)} placeholder="e.g. 250" className="border border-gray-300 outline-none rounded-lg h-[40px] pl-[10px]" />
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col mt-[40px] gap-[5px]">
                            ...
                            {ingredients.map((item, index) => (
                                <div key={index} className="flex items-center gap-[10px]">
                                    <input type="text" value={item} onChange={(e) => handleChange(ingredients, setIngredients, index, e.target.value)} placeholder={`Ingredient ${index + 1}`} className="border border-gray-300 outline-none rounded-lg w-full h-[40px] pl-[10px]" />
                        ...
                        <div className="flex flex-col mt-[40px] gap-[5px]">
                            ...
                            {instructions.map((step, index) => (
                                <div key={index} className="flex items-center gap-[10px]">
                                    <span>{index + 1}.</span>
                                    <textarea value={step} onChange={(e) => handleChange(instructions, setInstructions, index, e.target.value)} placeholder={`Step ${index + 1}`} className="border border-gray-300 outline-none rounded-lg w-full h-[60px] pl-[10px]" />
                            ...
                            <div className="flex flex-row flex-wrap gap-[20px]">
                                {tags.map((tag, index) => (
                                    <div key={index} className="flex items-center gap-[5px] w-[30%]">
                                        <input type="text" value={tag} onChange={(e) => handleChange(tags, setTags, index, e.target.value)} placeholder="e.g. vegetarian" className="border border-gray-300 outline-none rounded-lg w-full h-[50px] pl-[10px]" />
                                        {tags.length > 1 && <span className="text-[20px] cursor-pointer text-red-500" onClick={() => removeField(tags, setTags, index)}>×</span>}
                                    </div>
                                ))}
                            </div>
